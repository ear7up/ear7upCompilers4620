...
void ClassDecl::Check() {
    ...
    PrepareScope(); // sets up convImp, list of interface declarations implemented
    Type *unimplemented_interface_type = NULL;
    bool fully_implements = true;
    for (int n = 0; n < convImp->NumElements(); n++) {
	// look at each inherited interface
	InterfaceDecl *inherited_interface = dynamic_cast<InterfaceDecl*>(convImp->Nth(n));
	cout << "inherited interface: " << inherited_interface << "\n\n";
	// then look at each inherited function is implemented in the class
	if (inherited_interface) {
	    for (int n2 = 0; n2 < inherited_interface->members->NumElements(); n2++) {
	        FnDecl *inherited_function = dynamic_cast<FnDecl*>(inherited_interface->members->Nth(n2));
	        if (inherited_function) {

		    // try to find a function in the class scope with the same name as the inherited function
		    FnDecl *local_function = NULL;
		    if (members)
		    {
			for (int n3 = 0; n3 < members->NumElements(); n3++) {
			    local_function = dynamic_cast<FnDecl*>(members->Nth(n3));
			    if (local_function && !strcmp(local_function->GetName(), inherited_function->GetName())) break;
			}
		    }

		    // if the inherited function is not found in the class, we will report an error
		    if (!local_function or !local_function->MatchesPrototype(inherited_function)) {
			fully_implements = false;
			unimplemented_interface_type = implements->Nth(n);
			break;
		    }
		}
	    }
    ...
    if (!fully_implements) ReportError::InterfaceNotImplemented(this, unimplemented_interface_type);
